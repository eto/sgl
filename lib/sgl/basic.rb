...
module SGL
  class Timer	# starttime秒後に始まって、endtime秒後に終る。
    ...
    attr_accessor :st, :et
    def ratio	# start at 0.0, finished at 1.0
      count
    ...
    def revert; @bt = Time.now.to_f; end	# begintime→開始時間
    private def count; @now = Time.now.to_f - @bt; end
  end

  module Fadeout
    EXISTENCE_TIME = 6 # 生存時間
    FADEOUT_TIME = 2   # fadeoutする時間
    def fadeout_initialize;	@timer = Timer.new(EXISTENCE_TIME - FADEOUT_TIME, EXISTENCE_TIME);	end
    def set_longer_timer; t = 10; @timer = Timer.new(EXISTENCE_TIME * t - FADEOUT_TIME * t, EXISTENCE_TIME * t); end
    def fadeout; @timer.start; end
    def ended?;  @timer.ended?; end
  end
  ...
  class RingArray < Array
    ...
    def self.test	# RingArray.test
      t = RingArray.new
      t[0], t[1], t[2] = 0, 1, 2
      p [t[-3], t[-2], t[-1], t[0], t[1], t[2], t[3], t[4]]
      exit
    end
    def randomize!	# 適当な回数入れかえをして、ランダムな並びにする。
      loop {
	a = rand(self.length)
	b = rand(self.length)
	backup = self[a]
    ...
    def randomized?	# 最初にすでにランダム化されていることとする。
      randomized = true
      self.each_index {|i|
	c_1 = self[i-1]
	c   = self[i]
  ...
  class NumSpring
    EPSILON = 0.01
    def initialize(target, initial, ks, kd, e=EPSILON, v=0)	# 目標となる値、初期値、バネ定数、弾性定数、イプシロン(十分小さい値)
